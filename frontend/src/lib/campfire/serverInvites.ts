...
export function raffleReferrerEmail(opts: { name?: string | null; link: string }) {
  ...
  const hi = firstName(name ?? undefined);
  const subject = "🏆 New on Campfire: prize challenges & scavenger hunts (and a bigger cut for you)";
  ...
  const earnings =
    "💸 Why push these: you earn about 3% of every prize-event chip-in your linked groups make — three times the usual rate, added on top so it costs your groups nothing. Track it on your Campfire referral dashboard.";
