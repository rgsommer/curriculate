...
function linkifyTextToReactNodes(text) {
  ...
  return parts.map((part, i) => {
    ...
    // Because split() includes the matched URLs as elements,
    // we can detect URL parts by simple prefix checks (no regex .test needed)
    const looksUrl = part.startsWith("http://") || part.startsWith("https://") || part.startsWith("www.");
    if (!looksUrl) return <React.Fragment key={i}>{part}</React.Fragment>;

    // Trim common trailing punctuation that often sticks to URLs in prose
    const m = part.match(/^(.*?)([)\].,;:!?]+)?$/);
