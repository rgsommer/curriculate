...
import { kvGet, kvSet, reportKey, scanKey, orderKey } from '@/lib/businesses/store';
import { structured, REPORT_MODEL, estimateCostUsd } from '@/lib/businesses/anthropic';
import { REPORT_SYSTEM, reportPrompt } from '@/lib/businesses/prompts';
import { profileSchema, opportunitiesSchema, supportingSchema, conceptsSchema } from '@/lib/businesses/schemas';
import { siteUrl } from '@/lib/businesses/stripe';
import { sendReportReady } from '@/lib/businesses/email';
import type { FullReport, ScanResult, Order } from '@/lib/businesses/types';
...
export const POST = withApi(async (req: NextRequest) => {
  ...
  try {
    ...
    if (report.status === 'ready' && order?.email && !order.notified) {
      ...
      await sendReportReady(order.email, label, `${siteUrl()}/businesses/report/${reportId}`);
    }
