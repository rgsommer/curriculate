...
import { GAP_TYPE_LABEL, type FullReport, type Financials, type GapType } from '@/lib/opportunities/types';
...
export default function ReportView({ id }: { id: string }) {
  ...
  useEffect(() => {
    ...
    async function tick() {
      try {
        const res = await fetch(`/api/opportunities/report/${id}`, { cache: 'no-store' });
    ...
    <main className="mx-auto max-w-4xl px-5 py-12">
      <div className="no-print flex flex-wrap items-center justify-between gap-4">
        ...
        <div className="flex gap-2">
          ...
          <a href={`/api/opportunities/report/${id}`} download={`${r.city?.slug || 'report'}-data.json`} className="rounded-lg border border-slate-300 px-4 py-2 text-sm font-semibold text-slate-700 hover:bg-slate-50">Download data</a>
        </div>
      </div>
