...
| O Canada for (C) | The anthem holds the screen for the **last N minutes of the announcements window**, ending when it does (9:00 in the sheet) rather than running on past it — the words and the flag from the day's column of `Poems!F1:J3` — 5 if the row is missing, which it is in the sheet today. The minutes of the window before it carry the same screen rather than a blank one, and the teaching day (the first class, the Prayercast) starts at the end of the window, not five minutes later. The flag may be written into the cell or picked by a rule (`=IMAGE(INDEX(Flags!B:B, …))`), which is run at the board's clock |
...
| Stand ready for dismissal (C) | The moment the room has to be standing, where the row names it outright ("3:30 PM"). Without it the board uses the **home time** — the latest thing the sheet says about dismissal in so many words, which is the message block's own "Dismissal" entry. The note on the board names that moment: the last class ends at 3:25 and the dismissal screen comes up, but the homeroom teacher arrives at 3:30 |
