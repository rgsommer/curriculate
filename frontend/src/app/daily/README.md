...
| `SchoolCalendar!A1:G220` | The school's own calendar, a row per event: A the date in words, B the same date as `"<serial> n"` (the key the sheet's own lookups match, `n` telling two events on one day apart), C the event, D `TRUE` when school is closed, F a longer description. The banner across the top of the board says what is on today and what is on tomorrow, a day off in the alert colour. A2's notice rule reaches in here too, so naming the range makes that read certain instead of dependent on how the rule is written. |
