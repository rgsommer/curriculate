"use client";

import React, { useEffect, useMemo, useRef, useState } from "react";
import Link from "next/link";
import Lottie from "lottie-react";

import introAnim from "@/public/lottie/intro.json";
import buildAnim from "@/public/lottie/build.json";
import launchAnim from "@/public/lottie/launch.json";
import stationsAnim from "@/public/lottie/stations.json";
import captureAnim from "@/public/lottie/capture.json";
import reportAnim from "@/public/lottie/report.json";

type Step = {
  title: string;
  body: React.ReactNode;
};

const STORAGE_KEY = "curriculate_walkthrough_v1_dismissed";

function clamp(n: number, min: number, max: number) {
  return Math.max(min, Math.min(max, n));
}

function IconShell({
  accentText,
  accentBg,
  accentRing,
  animation,
}: {
  accentText: string;
  accentBg: string;
  accentRing: string;
  animation: object;
}) {
  return (
    <div className="mb-3 flex justify-center">
      <div
        className={`transition-colors duration-300 rounded-2xl p-3 ring-1 ${accentRing} ${accentBg}`}
      >
        <div className={`transition-colors duration-300 ${accentText}`}>
          <Lottie
            animationData={animation}
            loop
            autoplay
            style={{ width: 56, height: 56 }}
          />
        </div>
      </div>
    </div>
  );
}

export default function WalkthroughModal({
  open,
  onClose,
  startAt = 0,
  showDontShowAgain = true,
}: {
  open: boolean;
  onClose: () => void;
  startAt?: number;
  showDontShowAgain?: boolean;
}) {
  const steps: Step[] = useMemo(
    () => [
      {
        title: "What this is",
        body: (
          <div className="space-y-2">
            <p>
              Curriculate runs a simple loop: <b>Build</b> → <b>Launch</b> →{" "}
              <b>Stations</b> → <b>Capture</b> → <b>Reports</b>.
            </p>
            <p className="text-slate-600">
              This is a 60-second walkthrough. You’ll know the whole flow by the end.
            </p>
          </div>
        ),
      },
      {
        title: "1) Build a task set",
        body: (
          <div className="space-y-2">
            <p>
              Enter your topic, grade level, difficulty level, purpose, and (most
              importantly) paste in a list of words or concepts you want to teach or
              review.
            </p>
            <div className="rounded-2xl bg-slate-50 p-3 text-sm text-slate-700 ring-1 ring-black/5">
              Tip: Start with putting up <b>8 station posters</b>.
            </div>
          </div>
        ),
      },
      {
        title: "2) Launch a live session",
        body: (
          <div className="space-y-2">
            <p>
              Start the session on the teacher screen. Teams join quickly (scan/code).
              They enter a team name of their choice and their names. Pacing is fully
              automatic.
            </p>
            <div className="rounded-2xl bg-slate-50 p-3 text-sm text-slate-700 ring-1 ring-black/5">
              Everything is tracked while students work — no teacher busywork.
            </div>
          </div>
        ),
      },
      {
        title: "3) Stations + teamwork",
        body: (
          <div className="space-y-2">
            <p>
              Students rotate station-by-station. One device per team is enough—others
              can watch or assist.
            </p>
            <div className="rounded-2xl bg-emerald-50 p-3 text-sm text-emerald-900 ring-1 ring-emerald-200">
              <b>Devices:</b> Phones work, but the best experience is{" "}
              <b>~1 tablet per 3 students</b>.
            </div>
          </div>
        ),
      },
      {
        title: "4) Work is captured automatically",
        body: (
          <div className="space-y-2">
            <p>
              Answers are saved by station and task type. Optional scoring and feedback
              can be generated automatically.
            </p>
            <div className="rounded-2xl bg-slate-50 p-3 text-sm text-slate-700 ring-1 ring-black/5">
              No paper chase after class—everything is already organized.
            </div>
          </div>
        ),
      },
      {
        title: "5) Reports + reuse",
        body: (
          <div className="space-y-3">
            <p>
              End the session and get clear reporting for teachers and students.
              Reuse the task set next time with quick edits.
            </p>
            <div className="flex flex-wrap gap-2">
              <Link
                href="/how-it-works"
                className="rounded-2xl bg-white px-3 py-2 text-sm font-semibold text-slate-900 ring-1 ring-black/10 hover:bg-slate-50"
              >
                See the visual “How it Works” page
              </Link>
              <Link
                href="/demo"
                className="rounded-2xl bg-emerald-600 px-3 py-2 text-sm font-semibold text-white hover:brightness-105"
              >
                Try the demo
              </Link>
            </div>
          </div>
        ),
      },
    ],
    []
  );

  const animations = [
    introAnim,
    buildAnim,
    launchAnim,
    stationsAnim,
    captureAnim,
    reportAnim,
  ];

  const [idx, setIdx] = useState(startAt);
  const [dontShowAgain, setDontShowAgain] = useState(false);

  useEffect(() => {
    if (open) setIdx(startAt);
  }, [open, startAt]);

  const progress = Math.round(((idx + 1) / steps.length) * 100);

  function accentForIdx(i: number) {
    if (i >= 4)
      return {
        text: "text-rose-600",
        bar: "bg-rose-500",
        ring: "ring-rose-200",
        bg: "bg-rose-50",
      };
    if (i >= 2)
      return {
        text: "text-indigo-600",
        bar: "bg-indigo-500",
        ring: "ring-indigo-200",
        bg: "bg-indigo-50",
      };
    return {
      text: "text-emerald-600",
      bar: "bg-emerald-500",
      ring: "ring-emerald-200",
      bg: "bg-emerald-50",
    };
  }

  const accent = accentForIdx(idx);

  const [animateIn, setAnimateIn] = useState(false);
  const lastIdxRef = useRef(startAt);

  useEffect(() => {
    if (!open) return;
    if (lastIdxRef.current !== idx) lastIdxRef.current = idx;
    setAnimateIn(false);
    const t = setTimeout(() => setAnimateIn(true), 20);
    return () => clearTimeout(t);
  }, [open, idx]);

  if (!open) return null;

  const step = steps[idx];

  return (
    <div className="fixed inset-0 z-[100]">
      <button onClick={onClose} className="absolute inset-0 bg-black/40" />

      <div className="absolute inset-0 flex items-center justify-center p-4">
        <div className="w-full max-w-xl overflow-hidden rounded-3xl bg-white shadow-2xl ring-1 ring-black/10">
          {/* Header */}
          <div className="border-b px-6 py-4">
            <div className="text-xs font-semibold text-slate-500">
              Walkthrough • {idx + 1} of {steps.length}
            </div>
            <div className="mt-1 text-lg font-bold">{step.title}</div>
            <div className="mt-4 h-2 w-full rounded-full bg-slate-100">
              <div
                className={`h-2 rounded-full transition-all ${accent.bar}`}
                style={{ width: `${progress}%` }}
              />
            </div>
          </div>

          {/* Body */}
          <div className="px-6 py-5 text-sm">
            <div
              className={`transition-all duration-300 ${
                animateIn ? "opacity-100 translate-y-0" : "opacity-0 translate-y-2"
              }`}
            >
              <IconShell
                accentText={accent.text}
                accentBg={accent.bg}
                accentRing={accent.ring}
                animation={animations[idx]}
              />
              {step.body}
            </div>
          </div>

          {/* Footer */}
          <div className="flex justify-between border-t px-6 py-4">
            <button onClick={onClose} className="text-sm font-semibold text-slate-600">
              Skip
            </button>
            <button
              onClick={() =>
                idx === steps.length - 1 ? onClose() : setIdx((v) => clamp(v + 1, 0, steps.length - 1))
              }
              className="rounded-2xl bg-emerald-600 px-4 py-2 text-sm font-semibold text-white"
            >
              {idx === steps.length - 1 ? "Done" : "Next"}
            </button>
          </div>
        </div>
      </div>
    </div>
  );
}
