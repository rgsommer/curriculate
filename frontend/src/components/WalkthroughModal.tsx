...
export default function WalkthroughModal({
  ...
  // Lazy-load Lottie JSON only when the modal opens (files live in /public/lottie)
  ...
  useEffect(() => {
    ...
    Promise.all(files.map((p) => fetch(p).then((r) => r.json())))
      .then((arr) => {
        if (!cancelled) setAnimations(arr);
      })
      .catch(() => {
        if (!cancelled) setAnimations(null);
      })
      .finally(() => {
        if (!cancelled) setLoadingAnims(false);
      });
