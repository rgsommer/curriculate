    ...
    teams: (() => {
      ...
      for (const [teamId, t] of Object.entries(room.teams || {})) {
        ...
        out[teamId] = {
          ...
          isCohortMember: !!t.isCohortMember,
...
// ----------------------------------------------------
// Cohort pacing helpers (Option A)
// Rule: cohort teams may be at most 1 task ahead of the slowest cohort team.
// Late joiners can catch up but do not slow the cohort until they reach it.
// ----------------------------------------------------
function isTeamCohortMember(team) {
  return !!team?.isCohortMember;
}

function getCohortTeams(room) {
  return Object.values(room?.teams || {}).filter(isTeamCohortMember);
}

function getMinCohortTaskIndex(room) {
  const cohort = getCohortTeams(room);
  if (!cohort.length) return -1;
  let min = Infinity;
  for (const t of cohort) {
    const idx = (typeof t.taskIndex === "number") ? t.taskIndex : -1;
    if (idx < min) min = idx;
  }
  return Number.isFinite(min) ? min : -1;
}

// Can a team receive (be sent) a task with 'index' right now?
function canDeliverTaskIndex(room, team, index) {
  if (!room?.cohortPacing?.enabled) return true;

  // If team isn't in the cohort yet (late joiner catching up), allow delivery up to the cohort gate.
  const minCohort = getMinCohortTaskIndex(room);
  const gate = minCohort + 1;

  // If no cohort yet, allow.
  if (minCohort < 0) return true;

  if (!team?.isCohortMember) {
    // late team can catch up to the cohort (and optionally 1 ahead if you want).
    return index <= gate;
  }

  // cohort teams: never more than one ahead of the slowest cohort team
  return index <= gate;
}

// Promote a late joiner into the cohort once they've caught up
function maybePromoteToCohort(room, team) {
  if (!room?.cohortPacing?.enabled) return;
  if (!team || team.isCohortMember) return;
  const minCohort = getMinCohortTaskIndex(room);
  const idx = (typeof team.taskIndex === "number") ? team.taskIndex : -1;
  if (minCohort >= 0 && idx >= minCohort) {
    team.isCohortMember = true;
  }
}
    ...
    try {
      ...
      if (!room.teams[teamId]) {
        room.teams[teamId] = {
          ...
          taskIndex: -1,
                isCohortMember: !(room?.cohortPacing?.enabled && room?.isActive),
      };
        ...
        if (typeof room.teams[teamId].isCohortMember !== "boolean") {
          room.teams[teamId].isCohortMember = !(room?.cohortPacing?.enabled && room?.isActive);
        }
      }
  ...
  const handleStationScan = (payload = {}, ack) => {
    try {
      ...
      if (room.taskset && Array.isArray(room.taskset.tasks)) {
        ...
        if (queuedIndex >= 0) {
          // Cohort pacing gate: never allow > (min cohort + 1)
          if (!canDeliverTaskIndex(room, team, queuedIndex)) {
            const minCohort = getMinCohortTaskIndex(room);
            const gate = minCohort + 1;
            if (typeof ack === "function") {
              ack({
                ok: false,
                wait: true,
                error: `WAIT — another team is still working. Your team can go up to task ${gate + 1}.`,
              });
            }
            return;
          }
          ...
          // Late joiners become cohort members once they've caught up
          maybePromoteToCohort(room, team);
        }
      }
...
socket.on("station:scan", handleStationScan);
  ...
  function startTasksetForRoom(roomCode) {
    ...
    // Enable cohort pacing for this room and mark current teams as cohort members
    room.cohortPacing = { enabled: true, startedAt: Date.now() };

    // Reset per-team progress + cohort membership (true for currently joined teams)
    Object.values(room.teams || {}).forEach((team) => {
      team.taskIndex = -1;
      team.nextTaskIndex = null;
      team.isCohortMember = true;
    });
