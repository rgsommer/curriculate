...
5h. TRAILING STOP (per-holding daily): every technicals block includes "Trailing stop N% from 60d high $X → $Y · Z% slack". This is the actionable "sell if hit" line — a ratcheting stop anchored to the 60-session watermark high, using 2.5×ATR as the trail width. In the section-2 per-holding line (and in section 0b for a fresh BUY that just fulfilled a rec), CITE the trailing stop level explicitly: "Trail stop: $Y (Z% slack)" — quoting the numbers verbatim. If slack is ≤3%, add "⚠ approaching trail stop — trim or tighten manually." If STOP HIT flag appears, the position has already broken the trailing stop level; instruct SELL at market. Trailing stops beat static stops because they lock in gains as the price rises; users should be told the current level every day.
