...
import { getRubricById } from ".ai/rubrics.js";
import { scoreSubmissionWithAI } from "./ai/aiScoring.js";
...
io.on("connection", (socket) => {
    ...
    if (parsed) {
      ...
      if (taskset) {
        ...
        if (rawLocationCode !== locationCode) {
          const msg = `Wrong area. Use a "${locationCode}" station.`;  // ✅ new message
          console.log(
  ...
  socket.on(
    "submitTask",
    async ({ roomCode, correct, timeMs, memberName, teamId, answerText }) => {
      const code = (roomCode || "").toUpperCase();
      const room = rooms[code];
      if (!room) return;
      ...
      // 1) determine correctness (legacy)
      ...
      // 3) build submission object (for transcript + AI)
      const submission = {
        teamId: tid,
        ...
        taskIndex:
          typeof room.currentTaskIndex === "number"
            ...
            : 0,
        aiScore: null, // will be filled if AI scoring runs
      };

      // 3a) store once per team for this task for speed/bonus logic
      ...
      // 3b) also push into room.submissions for end-of-taskset transcript
      room.submissions = room.submissions || [];
      room.submissions.push(submission);

      // 3c) OPTIONAL AI scoring (e.g., open-text Quebec Act)
      try {
        const taskset = room.taskset;
        const currentIdx =
          typeof room.currentTaskIndex === "number"
            ...
            : 0;
        const currentTask =
          taskset?.tasks && taskset.tasks[currentIdx]
            ? taskset.tasks[currentIdx]
            : null;

        const aiRubricId = currentTask?.aiRubricId || null;
        const rubric =
          (aiRubricId && getRubricById(aiRubricId)) ||
          currentTask?.rubric ||
          null;

        const aiScoringEnabled =
          process.env.AI_SCORING_ENABLED !== "false" &&
          !!process.env.OPENAI_API_KEY;

        // For now, we mainly target text-based tasks (open-text).
        const isTextTask =
          currentTask &&
          (currentTask.taskType === "open-text" ||
            currentTask.taskType === "short-answer");

        if (aiScoringEnabled && rubric && isTextTask) {
          const aiResult = await scoreSubmissionWithAI({
            task: currentTask,
            rubric,
            submission,
          });
          ...
          submission.aiScore = {
            ...
            maxPoints: aiResult.maxPoints,
            criteria: aiResult.criteria || [],
            overallComment: aiResult.overallComment || "",
            model: process.env.AI_SCORING_MODEL || "gpt-4.1-mini",
            gradedAt: new Date().toISOString(),
          };
        }
      } catch (err) {
        console.error("AI scoring error (non-fatal):", err);
      }

      // 4) base scoring by team name (unchanged)
      ...
      // 5) relative speed bonus — ONLY for correct teams (unchanged)
      ...
      // 6) rotation for this team (unchanged)
      ...
      // 7) notify host/teacher (unchanged, but now includes answerText)
