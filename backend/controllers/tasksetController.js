...
export async function generateTaskset(req, res) {
  try {
    ...
    // -------------------------
    // Stage 3b: Reconcile task count
    // -------------------------
    const expectedCount = plannedTasks.length;
    let finalTasks = Array.isArray(cleanedTasks) ? cleanedTasks : [];

    // Trim extras (model sometimes overproduces)
    if (finalTasks.length > expectedCount) {
      finalTasks = finalTasks.slice(0, expectedCount);
    }

    // If AI under-produced, attempt ONE top-up generation
    if (finalTasks.length < expectedCount) {
      const missing = expectedCount - finalTasks.length;

      try {
        const topUpTasks = await createAiTasks({
          gradeLevel: effectiveConfig.gradeLevel,
          subject: effectiveConfig.subject,
          topicTitle: effectiveConfig.topicTitle,
          learningGoal: effectiveConfig.learningGoal,
          concepts: effectiveConfig.wordConceptList,
          taskPlan: plannedTasks.slice(-missing),
          curriculumLenses: effectiveConfig.curriculumLenses,
        });

        const cleanedTopUp = await cleanTaskList(topUpTasks, TASK_TYPES);
        finalTasks = finalTasks.concat(cleanedTopUp).slice(0, expectedCount);
      } catch (err) {
        console.warn(
          "[aiTasksetController] Task top-up failed:",
          err.message
        );
      }
    }

    // If still short, DO NOT 500 — save partial
