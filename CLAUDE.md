...
- The teacher's own material comes from the **Lessons** tab, keyed by the lesson code in column C (`~H001` or `H001`): E the starting page reference (a chip beside the room), F the homework (its links become handout chips, its text fills "Write in your agenda"), I the lesson picture, J the video when the DisplayAI row has none. The student-facing tabs leave all of this out on purpose (`parseLessons` in `lib/daily/parse.ts`). Columns I and J are the picture and video columns, so what they hold is what they are for: any URL there is taken, including a Drive link written `open?id=…` (which passes no image test) and a link attached to the cell's own text — the grid read covers `Lessons!E1:J400` for that. The picture takes the large side of the screen for the first `picSeconds` (10 min) of the class; `?debug=1` prints what was found and whether it loaded
