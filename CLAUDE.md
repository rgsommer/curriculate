...
- **Pictures the API cannot see.** The Sheets REST API has no image field at all: `CellData` carries formats, formulas, notes, hyperlinks and chips, `ExtendedValue` has no image type, and the discovery document has no image schema anywhere. A picture put *into* a cell with Insert > Image — either kind — therefore reaches the board as nothing, which is why the flag, the feature cartoon and the lesson pictures all read empty. The fix is in the sheet: `frontend/src/app/daily/apps-script/mirror-cell-images.gs` is an Apps Script that turns every in-cell picture into an `=IMAGE("…")` formula pointing at the same picture — the sheet looks the same, and the formula is readable. A picture inserted from a URL keeps that URL; one pasted or uploaded has its bytes copied to a Drive folder shared with anyone who has the link, because the projector is not signed in as the teacher and Google's own image URLs are tagged to the requester and expire within the hour
