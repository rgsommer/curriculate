...
- **The Setup slot rules are evaluated, not read** (`lib/daily/formula.ts`): each slot's row 4 in `Setup!S1:AB8` is a NOW()-driven rule — the CE memory verse for the first ten minutes, the picture in S2 on the week's last teaching day — and reading the cell gave the answer for the instant of the read, so the scrubber could not move E1. A small Sheets evaluator (IF/AND/OR, NOW, TIMEVALUE, WEEKDAY, INDEX, HLOOKUP, cell and range references) re-runs them with NOW() bound to the time on the board; anything it cannot follow, or that reaches past `Setup!A1:F20`, `Setup!M1:Q8`, `Setup!S1:AB8` or `Master!B1:B2`, falls back to the value the sheet computed. Row 3 is the slot's own material and row 4 the rule that gates it; `?debug=1` prints both for every slot
- The status is a privilege code shown as a coloured bubble mirroring the sheet's conditional formatting on D9/D11/D13 (`statusStyle`), and it says what the benefit is rather than the sheet's shorthand (`statusWords`, `BENEFIT_WORDS`): Benefit 1 "Sit anywhere", Benefit 2 "Washroom pass", Benefit 3 "Extra FD", all of them "All 3", a trailing 4 "+2" for a perfect class. The group letter sits small beside the words and the code stays in the tooltip. A code with no text label (`A-1000`) is read as its digits, in the legend's order — Benefit 1, 2, 3 — instead of reading as nothing. The washroom pass is shut at both ends of the period — for the opening minutes of the lesson nobody walks out, which is Setup's grace window and the same one the sheet's status rule uses to force B2 off, and again from Setup's "Can go to washroom x min before" — and while it is shut at the top the chip counts down to when it opens
...
- The points strip names its chips from Points row 3, so it shows the sections actually taught this year rather than a fixed 7A–8C list; the "Plans for …" line writes each class's total as `-value-`, and neighbours may share the dash between them, so the values are split on the dash (a run of three dashes is a negative value) rather than matched — matching swallowed the dash that started the next value and showed four classes as two
