...
- Handouts for the class on screen show as clickable chips under the lesson bullets, so a print run that was forgotten can be started from the board. They come from URLs written into the lesson cell (`extractLinks`) and from rich-text links attached to phrases in it (`readCellLinkRuns` over `DisplayAI!C1:C40`); the name is taken from the words that introduced the link, falling back to the kind of file. The addresses are stripped out of the bullets, which also stops a URL's own `?` being read as the lesson's question
...
- When DisplayAI's lesson column is empty — it fills from the sheet's own clock, so before the day starts it can be blank — the board falls back to the day's plan on **VerticalAi columns F to J** (Monday to Friday, the day's classes run together in one cell): `classesFromText` finds each class header and parses the chunk after it, `dayPlanByWeekday` does it per weekday. The lesson code is optional in a class header, because that text writes some classes without one
