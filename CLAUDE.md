...
- **When those cells read nought, the board adds the Points tab up itself** (`pointsFromGrid`). Each class has a block on that tab: the name in row 3 at D/Q/AD/AQ/BD/BQ, its target two columns on (240 for 7A, 320 for 7B, 160, 400, 160), the days it meets one further on again, then a week per row from row 6 — Monday to Friday, and a Total column the sheet computes. Setup rows 35 to 40 are built from those Totals, and they broke: row 5 of the tab is `#REF!` and every weekly Total reads 0 while the day cells still hold 10, 10, 10, so the sheet's own plans line and the strip both showed five classes on 0%. The days are therefore added up here and the percentage taken against the class's own target — 7A's 30 out of 240 is the 13% the line used to show. It is a fallback, not a replacement: the moment Setup's own cells carry figures again they are what shows, and `?debug=1` says which of the two the figures came from
