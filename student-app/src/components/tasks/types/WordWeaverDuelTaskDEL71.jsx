...
export default function WordWeaverDuelTask({
  ...
  const scrabbleWords = useMemo(() => {
    const raw = Array.isArray(task?.words) ? task.words : Array.isArray(task?.config?.words) ? task.config.words : [];
    return raw.map((w) => String(w || "").trim()).filter(Boolean).slice(0, 10);
  }, [task]);
        ...
        <div style={{ marginTop: 12 }}>
          ...
          <div style={styles.controls}>
            <button
              ...
              title="Submit the final score snapshot"
            >
              Submit ✅
            </button>
    ...
    return (
      <div style={styles.card}>
        ...
        <div style={styles.muted}>
          Missing phrase. Expected <code>task.phrase</code> or <code>task.targetPhrase</code>, or provide <code>task.words</code> (5–10) for Scrabble mode.
        </div>
      </div>
    );
  }
