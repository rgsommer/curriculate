...
export default function MatchingTask({ task, onSubmit, disabled, onAnswerChange }) {
  ...
  const normalized = useMemo(() => {
    ...
    if (leftRaw && rightRaw) {
      ...
      rightItems = pairsRaw.map((p, idx) => ({
        ...
        label: norm(
          p?.rightLabel ?? p?.rightText ?? p?.right ?? p?.definition ?? `Right ${idx + 1}`
        ),
      }));
  ...
  const getRightForLeft = (leftId) => matches[leftId] || null;
  ...
  // Simple line drawing: compute midpoints of matched buttons
  ...
  useEffect(() => {
    const compute = () => {
      ...
      for (const [leftId, rightId] of Object.entries(matches)) {
        ...
        const ok = correctMatches?.[leftId] === rightId;

        next.push({ x1, y1, x2, y2, ok });
      }
      setLines(next);
    };
        ...
        <svg ref={svgRef} className="absolute inset-0 w-full h-full pointer-events-none">
          {lines.map((ln, idx) => (
            <line
              ...
              strokeWidth="4"
              strokeLinecap="round"
              stroke={ln.ok ? "rgba(34,197,94,0.85)" : "rgba(59,130,246,0.75)"}
            />
          ))}
        </svg>
            ...
            {leftItems.map((it) => {
              ...
              return (
                ...
                >
                  {it.label}
                  {chosen && (
                    <span className="ml-2 text-xs font-extrabold text-slate-400">
                      ✓
                    </span>
                  )}
                </button>
