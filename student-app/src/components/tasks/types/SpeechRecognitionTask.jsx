...
export default function SpeechRecognitionTask({ task, onSubmit, disabled = false, presenter }) {
  ...
  const targetText = task?.config?.referenceText || task?.referenceText || task?.config?.prompt || task?.prompt;
  const language = task?.config?.language || task?.language || "en-US";
  ...
  const handleSubmit = () => {
    // Use the combined transcript (final + interim) so the submit button
    // behavior matches what the student sees on screen.
