 ...
 */
export default function ScriptPlayTask({ task, onSubmit }) {
  ...
  function goPrev() {
    ...
    // move to previous scene, last turn
  ...
  // Minimal CSS-in-JS to avoid needing global styles, but still "Curriculate rich"
  const styles = {
    wrap: {
      borderRadius: 18,
      padding: 16,
      ...
      <div style={styles.sceneCard}>
        ...
        <div style={styles.lineCard}>
          <div style={styles.speakerRow}>
            ...
            <div style={styles.speakerHint}>Pass the device to Speaker {((turn.speakerIndex + 1) % playerCount) + 1} next</div>
          </div>
          ...
          {!!turn.before && <div style={styles.smallContext}><b>Just before:</b> {turn.before}</div>}

          <div style={styles.lineText}>{turn.line}</div>

          {!!turn.after && <div style={styles.smallContext}><b>Up next:</b> {turn.after}</div>}
        </div>
        ...
        <div style={styles.controls}>
          <button type="button" style={styles.btn} onClick={goPrev} disabled={!canGoPrev}>
            ◀ Back
          </button>
