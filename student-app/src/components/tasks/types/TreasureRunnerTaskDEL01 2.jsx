...
export default function TreasureRunnerTask({
  ...
  function animate() {
    ...
    s.obstacles = s.obstacles.filter((o) => {
      ...
      if (hit && s.shieldMs <= 0) {
        s.hits += 1;
        setCurrentScore(p => Math.max(0, p - 3));
        s.vx *= 0.65;
        vibrate([100, 50, 100]);
        playSound("hit");
        s.shieldMs = 1200;
    ...
    ctx.fillText(`Score: ${currentScore}`, 12, 94);
      ...
      {permission !== "granted" && (
        <div style={{ marginTop: 10, padding: 12, background: "rgba(255,255,255,0.1)", borderRadius: 12 }}>
          ...
          <div style={{ fontSize: "0.9rem", opacity: 0.9 }}>
            Tap to jump • Drag left/right for speed • Drag up/down for lanes • Tilt for extra control
          </div>
        </div>
      )}
